...
public class JPushReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        if (JPushInterface.ACTION_NOTIFICATION_RECEIVED.equals(intent.getAction())) {
            Toast.makeText(context, "推送成功_上", Toast.LENGTH_SHORT).show();
        ...
        // 需要跳转的action
            ...
            Toast.makeText(context, "推送成功_下", Toast.LENGTH_SHORT).show();

            /**
             * 此处可以通过写一个方法，决定出要跳转到那些页面，一些细节的处理，可以通过是不是从推送过来的，去多一个分支去处理。
             * 1.应用未启动,------>启动主页
             *      ----->不需要登陆信息类型，直接跳转到消息展示页面
             *      ----->需要登陆信息类型，由于应用都未启动，肯定不存在已经登陆这种情况
             *      ------>跳转到登陆页面
             *                                                                                                 ----->登陆完毕，跳转到信息展示页面。
             *                                                                                                 ----->取消登陆，返回首页。
            ...
            // 应用没有启动。。。
                ...
                if (true) {
                    ...
                    Intent pushMessageIntent = new Intent(context, PushMessageActivity.class);
                    //pushMessageIntent.putExtra("fromPush", true);
                    pushMessageIntent.putExtra("pushMessage", "message");

                    context.startActivities(new Intent[]{mainIntent, pushMessageIntent});
                }
    ...
    private boolean getCurrentTask(Context context) {
        ...
        List<ActivityManager.RunningTaskInfo> appProcessInfos =
